...
- **顶部栏**：左侧下拉选择 AI 角色，中间下拉选择/管理会话，右侧按钮切换模型
- **消息区**：显示对话历史，支持 Markdown 渲染
- **输入区**：文本输入框 + 附件按钮（📎）+ 图片按钮 + 发送按钮（🚀）
...
在对话过程中可以随时切换底层 AI 模型：

1. 点击顶部栏右侧的 ⚙️ 模型切换按钮
2. 从可用模型列表中选择一个模型
3. 可选：调整**思考等级**（影响 AI 推理的深度和耗时）
   - 不同等级消耗的 token 量不同，等级越高思考越深入

> **贴士**：如果某模型响应较慢或质量不理想，随时可以切换其他模型，对话上下文会保留。
...
2. 填写以下信息：
   - **标题**：知识点的名称
   - **正文**：知识点的详细内容（支持 Markdown 格式，可使用 `[[双链]]` 语法关联其他卡片）
   - **标签**：用逗号分隔的关键词标签
   - **生命周期**：选择衰减速度（见下文）
3. 点击保存
