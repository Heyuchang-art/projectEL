...
点击左侧边栏底部的 ⚙️ **齿轮图标**，右侧会滑出系统设置面板。

### 6.2 管理服务商与 API Key

系统内置了多个常见的 AI 服务商，同时也支持自定义添加服务商。

**服务商启用与状态控制：**
- **状态切换**：每个服务商右侧配有复古的 **Switch 状态开关**，可随时点击启用或禁用该服务商下的所有模型。
- **配置要求**：必须填入对应的 **API Key** 才可以开启该服务商的开关。若未填入 Key，系统会显示 **`未配置 Key`** 标签，且强制保持在禁用状态。已禁用的服务商会显示 **`已禁用`** 标签。
- **存储安全**：所有 API Key 仅保存在本地的 `.pi/auth.json` 中，绝不会上传到第三方。系统启动时会自动清理空值，以启用环境变量备份。

**自定义与默认服务商管理：**
- **添加自定义服务商**：点击设置面板最上方的 **“+ 添加自定义服务商”** 按钮，在弹出的窗口中填入服务商 ID、显示名称、协议类型（`openai-completions` 或 `anthropic-messages`）、Base URL（API 地址）及 API Key 即可。
- **删除与恢复默认服务商**：在服务商卡片上点击删除图标即可将其移除。若需要一键还原系统默认服务商配置，可点击面板底部的 **“恢复默认服务商”** 按钮进行一键复原（注意：内置的 OpenRouter 服务商已移除，可通过自定义方式添加）。

### 6.3 行内模型管理

在各个服务商卡片内部，你可以展开折叠列表并对该服务商的模型进行管理：

- **模型启用开关**：每个模型右侧也拥有独立的 **Switch 开关**。当服务商整体处于启用状态时，可以单独开关某个模型。若某个服务商被整体禁用，其下属的所有模型均会置灰。
- **添加模型**：点击模型列表底部的 **“+ 添加模型”** 按钮，填入模型 ID、显示名称并选择是否开启 Reasoning（推理功能支持）后保存，即可动态扩增该提供商的模型选择范围。
- **删除模型**：在模型列表行内，点击 **“🗑️ 垃圾桶”** 按钮可直接将此模型移除。

> **⚠️ 注意**：设置面板已完成与 Chat 交互逻辑的解耦，你无需在此处选择“当前活跃模型”。保存设置后，请直接在 **Chat 卡片的头部控制栏** 下拉选择活跃模型与推理等级，已禁用的服务商/模型将自动从 Chat 选择器中隐藏。
